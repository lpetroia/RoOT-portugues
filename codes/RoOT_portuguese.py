## code for defining delivery routes using index method
...
root.msg = Label(root, text="Ferramenta para otimização de rotas (RoOT)")
...
buttonChooseFile = Button(root, text='Escolha um arquivo de entrada para a otimização de rotas',
                           command=lambda: choose_file(lblFileName))

buttonChooseFile.pack(pady=10)

buttonCommit = Button(root, height=1, text="Rode a otimização de rotas",
...
output_name = inputValue.split(".")[0] + '_resultado_'+ str(date.today()) +'.xlsx'
# print(output_name)

# print('RoOT was developed at the University of Washington and VillageReach')
print('Carregando dados, por favor espere, isso levará alguns segundos...')

data = pd.ExcelFile(file_name)

data_capacities = pd.read_excel(data, 'capacidades_unidades_sanitarias', 0, index_col=None,
                                na_values=['NA'], skiprows=1)

data_demand = pd.read_excel(data, 'demanda', 0, index_col=None, skiprows=2)

data_products = pd.read_excel(data, 'produtos', 0, index_col=None,
                              na_values=['NA'], skiprows=1)
data_transit = pd.read_excel(data, 'distancias', 0, index_col=None,
                             na_values=['NA'], skiprows=2)
data_vehicle = pd.read_excel(data, 'veiculo', 0, index_col=None,
                             na_values=['NA'], skiprows=1)
data_input = pd.read_excel(data, 'parametros', 0, index_col=None, na_values=['NA'])

# data_arcs = pd.read_excel(data, 'available_roads', 0, index_col=None, na_values=['NA'],
#                           skiprows=2)
data_arcs = pd.read_excel(data, 'condicao_da_estrada', 0, index_col=None, na_values=['NA'],
                          skiprows=2)
...
#         print(routes)
    ...
    worksheet = workbook.add_worksheet('Rotas')
    ...
    worksheet.write('A1', 'DESCRICÃO DA ROTA:', gray)
    worksheet.merge_range('B1:D1', Description, merge_format)
    worksheet.write('F1', "RESULTADO - PLANILHA 1 OF 2", gray)
    
    worksheet.merge_range('A3:C3', 'SUMÁRIO DA ROTA E DISTRIBUIÇÃO DE PRODUTOS', gray)
    worksheet.write('A5', 'DISTÂNCIA TOTAL (Km):', gray)
 
    worksheet.write('B5', 'CUSTO TOTAL DE COMBUSTÍVEL:', gray)
 
    worksheet.write('C5', 'CUSTO TOTAL DE PER DIEM:', gray)
 
    worksheet.write('D5', 'CUSTO TOTAL (COMBUSTÍVEL + PER DIEM):', gray)
 
    worksheet.write('E5', 'TOTAL DE DOSES ENTREGUES:', gray)
 
    worksheet.write('F5', 'CUSTO POR DOSE:', gray)
    
    worksheet.merge_range('A8:C8', 'INFORMAÇÃO DETALHADA DA ROTA PARA '+
                          str(len(routes)) +" ROTAS",gray)
    ...
    for key in routes.keys():
        ...
        worksheet.write_string(row, col, 'ROTA:', gray)
        worksheet.write_string(row + 1, col, key, bold)
 
        worksheet.write_string(row, col + 1, 'VEÍCULO:', gray)
        worksheet.write_string(row + 1, col + 1, routes[key][0], bold)
 
        worksheet.write_string(row, col + 2, 'CONDIÇÃO DO VEÍCULO:', gray)
        worksheet.write_string(row + 1, col + 2, list_pen_cars[int(Beta[routes[key][0]]) - 1], bold)
 
        worksheet.write_string(row, col + 3, 'DISTÂNCIA DA ROTA (Km):', gray)
 
        worksheet.write_string(row, col + 4, 'CUSTO DE COMBUSTÍVEL DA ROTA:', gray)
 
        worksheet.write_string(row, col + 5, 'CUSTO PER DIEM DA ROTA:', gray)
        worksheet.write_number(row + 1, col + 5, cost_personnel[routes[key][0]], simple)
 
        worksheet.write_string(row, col + 6, 'TOTAL DE DOSES ENTREGUE:', gray)
        worksheet.write_string(row, col + 7, 'CUSTO TOTAL POR DOSE:', gray)
 
        worksheet.write_string(row, col + 8, 'UTILIZAÇÃO REFRIGERADA DO VEÍCULO (%):', gray)
 
        worksheet.write_string(row, col + 9, 'UTILIZAÇÃO NÃO REFRIGERADA DO VEÍCULO (%):', gray)
        ...
        worksheet.write_string(row, col + 10, 'UNIDADES SANITÁRIAS:', gray)
        ...
        worksheet.write_string(row, col + 11, 'HORA PARA SAIR DA UNIDADE SANITÁRIA:', gray)
 
        worksheet.write_string(row, col + 12, 'CONDIÇÃO DA ESTRADA:', gray)
    ...
    worksheet2 = workbook.add_worksheet('Produtos')
    worksheet2.set_column(0, 30, 19)
    worksheet2.write('A1', 'DESCRIÇÃO DA ROTA', gray)
    worksheet2.merge_range('B1:D1', Description, merge_format)
    worksheet2.write('F1', "RESULTADO - PLANILHA 2 DE 2", gray)
    worksheet2.merge_range('A3:B3', 'PRODUTOS ENTREGUES', gray)
 
    row = 4
    col = 0
 
    for key in routes.keys():
        worksheet2.write_string(row, col, 'ROTA:', gray)
        worksheet2.write_string(row, col + 1, key, bold)
        worksheet2.write_string(row + 1, col, 'VEÍCULO:', gray)
        worksheet2.write_string(row + 1, col + 1, routes[key][0], bold)
        worksheet2.write_string(row + 2, col, 'LOCAL INICIAL::', gray)
        worksheet2.write_string(row + 2, col + 1, origin, bold)
        row = row + 4
        for i in range(2, len(routes[key])):
            worksheet2.write_string(row, col, 'UNIDADE SANITÁRIA:', gray)
            worksheet2.write_string(row, col + 1, routes[key][i], bold)
            worksheet2.write_string(row + 1, col, 'UTILIZAÇÃO DE CAPACIDADE REFRIGERADA NA UNIDADE SANITÁRIA (%):', gray)
            worksheet2.write_number(row + 1, col + 1, round((d[routes[key][i], 'cold'] /
                                                             r[routes[key][i]]) * 100, 2), bold)
            worksheet2.write_string(row + 2, col, 'UTILIZAÇÃO DE CAPACIDADE NAO REFRIGERADA NA UNIDADE SANITÁRIA (%):', gray)
            worksheet2.write_number(row + 2, col + 1, round((d[routes[key][i], 'dry'] /
                                                             dry[routes[key][i]]) * 100, 2), bold)
            worksheet2.write_string(row + 3, col, 'PRODUTO:', gray)
            worksheet2.write_string(row + 3, col + 1, 'QUANTIDADE (DOSE OU UNIDADES):', gray)
            row = row + 3
    ...
    root2.title("RoOT - portugues")
 
    final_text = "Ferramenta par otimização de rotas (RoOT)\n \n  Seus resultados estão em:\n \n" + output_name
 
    msg = Message(root2, text=final_text, width=500, justify=CENTER)
 
    closing_button = Button(root2, height=1, width=10,
                            text="Fechar janela", command=root2.destroy)
